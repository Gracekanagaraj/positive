...
{
    if(root->left==NULL){root->left=new Node(k); return;}
    if(root->right==NULL){root->right=new Node(k); return;}
    if(k<=root->data1){insert(root->left,k); }
